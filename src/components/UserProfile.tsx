  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card rounded-3xl p-5 sm:p-8 shadow-[var(--shadow-card)]">
        <div className="flex items-start gap-5">
          ...
          <div className="relative group">
            <div className="w-20 h-20 rounded-2xl bg-rose-100 overflow-hidden shadow-md">
              {photoURL ? (
          ...
          <div className="flex-1 min-w-0">
              ...
              {isEditingName ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="bg-white border border-rose-300 rounded-lg px-2 py-1 text-lg font-bold text-gray-900 focus:outline-none focus:ring-2 focus:ring-rose-500 w-40"
                    maxLength={20}
            ...
            <div className="flex flex-wrap gap-1.5 mb-3">
              {userProfile?.titleLevel1 && (
                <span className="px-2 py-0.5 bg-rose-50 text-rose-500 text-xs font-medium rounded-lg border border-rose-200">{userProfile.titleLevel1}</span>
              )}
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: uiLang === 'zh' ? '词条贡献' : 'Contributions', value: userProfile?.approvedSlangCount || 0, color: 'text-rose-500', bg: 'bg-rose-50' },
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-1.5 group relative rounded-2xl p-2 transition-all ${
                  isEquipped ? 'bg-rose-50 ring-2 ring-blue-400 shadow-sm' : 'hover:bg-gray-50'
                } ${!unlocked ? 'cursor-not-allowed' : 'cursor-pointer'}`}
              >
                <AchievementBadge achievement={achievement} unlocked={unlocked} size="md" />
                <span className={`text-[10px] font-medium text-center leading-tight ${isEquipped ? 'text-rose-500 font-bold' : unlocked ? 'text-gray-700' : 'text-gray-400'}`}>
        ...
        <div className="flex border-b border-gray-100">
          {[
            ...
            <button
              ...
              className={`flex-1 py-3 text-sm font-semibold transition-colors relative ${activeTab === tab.id ? 'text-rose-500' : 'text-gray-400 hover:text-gray-600'}`}
            >
              {tab.label}
              {activeTab === tab.id && (
                <motion.div layoutId="profileTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-rose-500" />
              )}
            </button>
          ))}
        </div>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                    <div className="flex items-center justify-between">
                      ...
                      {!userProfile?.isPro && (
                        <button onClick={() => onOpenPayment('default')} className="bg-rose-500 text-white text-xs font-bold px-3 py-1.5 rounded-lg hover:bg-rose-600 transition-colors">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 p-3 bg-rose-50 text-rose-500 hover:bg-rose-100 rounded-xl transition-colors text-sm font-bold mt-2"
                    >
