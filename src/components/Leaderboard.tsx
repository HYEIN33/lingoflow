...
export default function Leaderboard({ defaultTab = 'group', currentUserId, uiLang = 'zh', onUserClick, onContribute }: { defaultTab?: 'group' | 'global' | 'monthly', currentUserId: string, uiLang?: 'en' | 'zh', groupId?: string, onUserClick?: (uid: string) => void, onContribute?: () => void }) {
  ...
  const renderGroupTab = () => (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
      {/* Timer */}
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-violet-100 rounded-2xl p-4 flex items-center justify-between">
        <div>
          <p className="text-violet-400 text-xs font-medium mb-1">{uiLang === 'zh' ? '本周结算倒计时' : 'Weekly reset'}</p>
          <p className="text-gray-900 font-mono font-bold text-lg flex items-center gap-2">
            <Clock className="w-4 h-4 text-violet-400" /> {timeLeft}
          </p>
        </div>
        ...
        {groupData.map((user, index) => (
          <motion.div
            ...
            className={`flex items-center gap-3 p-4 rounded-2xl cursor-pointer transition-all ${
              user.id === currentUserId
                ? 'bg-violet-50/80 border-2 border-violet-300 shadow-md shadow-violet-100/50'
                : index === 0
          ...
          >
            ...
            <div className={`w-10 h-10 rounded-xl overflow-hidden shrink-0 flex items-center justify-center font-bold ${
              index === 0 ? 'bg-amber-100 text-amber-700' : index === 1 ? 'bg-gray-100 text-gray-600' : index === 2 ? 'bg-orange-100 text-orange-700' : 'bg-violet-100 text-violet-500'
            }`}>
            ...
            <div className="flex-1 min-w-0">
              <h4 className="text-gray-900 font-bold truncate flex items-center gap-2">
                {user.username}
                {user.id === currentUserId && <span className="text-[10px] bg-violet-500 text-white px-2 py-0.5 rounded-full font-bold shadow-sm">YOU</span>}
              </h4>
      ...
      {globalData.map((user, index) => (
        <motion.div
          ...
          className={`flex items-center gap-3 p-4 rounded-2xl cursor-pointer transition-all ${
            ...
            user.id === currentUserId ? 'bg-violet-50/80 border-2 border-violet-300 shadow-md shadow-violet-100/50' :
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-md ${
                ...
                index === 1 ? 'bg-gradient-to-br from-blue-400 to-fuchsia-300' :
    ...
    <div className="space-y-6">
        ...
        {[
          ...
          <button
            ...
            className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all ${
              activeTab === tab.id
                ? 'bg-white/70 text-violet-500 shadow-sm backdrop-blur-md border border-white/60'
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-gray-400">
          ...
          {onContribute && (
            <button onClick={onContribute} className="mt-4 px-4 py-2 bg-violet-500 text-white text-sm font-medium rounded-xl hover:bg-violet-600 transition-colors">
