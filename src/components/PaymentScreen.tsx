      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm">
        ...
        >
          <motion.div
            ...
            className="w-24 h-24 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg shadow-violet-100"
          >
      ...
      >
        ...
        <div className="p-8 text-center border-b border-gray-100 bg-gradient-to-b from-blue-50 via-indigo-50/30 to-transparent">
          ...
          <div className="flex items-center justify-center gap-4 mt-8">
            ...
            >
              <motion.div animate={{ x: isYearly ? 24 : 0 }} className="w-6 h-6 bg-violet-500 rounded-full shadow-lg" />
            </button>
        ...
        <div className="p-8 grid md:grid-cols-2 gap-6">
          ...
          <motion.div
            ...
            className={`rounded-3xl p-5 sm:p-8 border-2 relative transition-all cursor-pointer ${
              selectedPlan === 'pro'
                ? 'bg-gradient-to-br from-blue-50 via-white to-indigo-50 border-violet-400'
                : 'bg-white/60 border-gray-100 hover:border-violet-200 hover:bg-violet-50/30'
            }`}
          >
            <motion.div
            ...
            <h3 className="text-xl font-bold mb-2 flex items-center gap-2">
              <span className="gradient-text">Pro</span> <Zap className="w-5 h-5 text-violet-500 fill-blue-600" />
            </h3>
            <div className="flex items-baseline gap-1 mb-6">
              <span className="text-3xl font-black bg-gradient-to-r from-violet-500 to-fuchsia-400 bg-clip-text text-transparent">¥{isYearly ? '28' : '48'}</span>
            ...
            <ul className="space-y-3 text-sm text-gray-700">
              <li className="flex items-center gap-3"><div className="w-5 h-5 rounded-full bg-violet-100 flex items-center justify-center shrink-0"><Check className="w-3 h-3 text-violet-500" /></div> {uiLang === 'zh' ? '无限次 AI 翻译 + 语法检查' : 'Unlimited AI translation + grammar'}</li>
              <li className="flex items-center gap-3"><div className="w-5 h-5 rounded-full bg-violet-100 flex items-center justify-center shrink-0"><Check className="w-3 h-3 text-violet-500" /></div> {uiLang === 'zh' ? '语气滑块（正式程度调节）' : 'Formality slider'}</li>
              <li className="flex items-center gap-3"><div className="w-5 h-5 rounded-full bg-violet-100 flex items-center justify-center shrink-0"><Check className="w-3 h-3 text-violet-500" /></div> {uiLang === 'zh' ? '艾宾浩斯复习系统（SM-2）' : 'Spaced repetition (SM-2)'}</li>
              <li className="flex items-center gap-3"><div className="w-5 h-5 rounded-full bg-violet-100 flex items-center justify-center shrink-0"><Check className="w-3 h-3 text-violet-500" /></div> {uiLang === 'zh' ? '优先 AI 响应速度' : 'Priority AI response'}</li>
              <li className="flex items-center gap-3"><div className="w-5 h-5 rounded-full bg-violet-100 flex items-center justify-center shrink-0"><Check className="w-3 h-3 text-violet-500" /></div> {uiLang === 'zh' ? '全球排行榜参与资格' : 'Global leaderboard access'}</li>
            </ul>
          </motion.div>
        </div>
            ...
            >
              <div className="border-t border-gray-100 pt-8">
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center justify-center gap-2 p-4 rounded-xl border-2 transition-all ${
                        paymentMethod === method.id ? 'bg-violet-50 border-violet-400 text-violet-500' : 'bg-gray-50 border-gray-100 text-gray-500 hover:border-gray-200'
                      }`}
                    >
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-6 space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        autoComplete="off"
                        className="w-full bg-white border border-gray-200 rounded-lg text-gray-900 p-3 focus:outline-none focus:ring-2 focus:ring-violet-500 font-mono"
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs text-gray-500 mb-1">{uiLang === 'zh' ? '有效期' : 'Expiry'}</label>
                        <input type="text" placeholder="MM/YY" autoComplete="off" className="w-full bg-white border border-gray-200 rounded-lg text-gray-900 p-3 focus:outline-none focus:ring-2 focus:ring-violet-500 font-mono" />
                      </div>
                      <div>
                        <label className="block text-xs text-gray-500 mb-1">CVV</label>
                        <input type="password" placeholder="123" autoComplete="off" maxLength={4} className="w-full bg-white border border-gray-200 rounded-lg text-gray-900 p-3 focus:outline-none focus:ring-2 focus:ring-violet-500 font-mono" />
                      </div>
                    </div>
                  </motion.div>
                )}
                ...
                <button
                  ...
                  onClick={handlePay}
                  className="w-full bg-gradient-to-r from-violet-500 to-fuchsia-400 hover:from-blue-700 hover:to-indigo-700 disabled:from-gray-200 disabled:to-gray-200 disabled:text-gray-400 text-white font-bold py-4 rounded-2xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-violet-200/50 hover:shadow-xl hover:shadow-blue-300/50"
                >
