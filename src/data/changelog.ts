...
export const CHANGELOG: ChangelogEntry[] = [
  {
    version: '0.2.14',
    date: '2026-04-20',
    title: '修了两个 bug：梗百科点赞和生词本搜索',
    changes: [
      '梗百科点赞修好了：之前点任何一个梗的点赞按钮都会静默失败（权限错误），现在能正常点赞了',
      '生词本搜索不再被其他页面串页：之前从翻译页点「查看百科详情」跳梗百科后，再切生词本会看到搜索框里残留之前的搜索词，导致「没有找到匹配的单词」让人以为收藏丢了，现在切到生词本会自动清空搜索',
      '课堂同传往上翻历史不再被新字幕拽回底部：在底部时新字幕出来会照常自动滚；往上滑查看前面讲过的内容时，新字幕会加到下面但你的视图不动',
    ],
    isMajor: false,
  },
  {
    version: '0.2.1',
    date: '2026-04-20',
    title: '课堂同传：更稳、更准，能选课，支持两种翻译模式',
    changes: [
      '实时翻译课堂内容，支持网课（Zoom / Teams 标签页）和线下课（麦克风）两种场景',
      '开课前可以选一门课（金融 / 计算机 / 法律 / 传媒 / 哲学……），AI 按专业场景翻译术语',
      '两种翻译模式可切：整段翻译（几句攒成一段再翻，读起来顺）/ 实时翻译（每句立刻翻，延迟最低）',
      '整段翻译累积 180 字或停顿 4.5 秒就翻，最长兜底 18 秒；中文以段落形式一次出现，读起来像文章不像字幕',
      '翻译中会显示「翻译中…」，不会让人怀疑是不是卡了',
      '课堂里有「暂停」按钮，中场休息按一下不用重新开始',
      '听不懂直接在底部「问 AI」打字提问，它会根据老师刚讲的内容回答',
      '下课自动把笔记保存到云端，支持翻阅、改名、分文件夹收纳',
      'Tab 条支持横向滑动，长按当前 Tab 才能拖动排序',
      '翻译偶尔失败时自动重试，减少漏翻',
    ],
    isMajor: true,
  },
